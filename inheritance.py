...
#다중 상속
            # 부모클래스가 한 개가 아닌 여러 개이다. 이러한 기법을 다중 상속이라고 한다.
...
# 생성자
            ...
            # 자식클래스에는 기본 생성자가 있다면 부모 클래스 생성자를 자동으로 호출해준다.
            #하지만 자식 클래스에서 생성자를 직접 선언하면 부모 생성자를 자식 생성자에서 작접 호출해 주어야 한다.
...
#자식 클래스에 생성자가 없을 때( 기본 생성자만 있을때)
            #부모의 필드 외에 추가적인 필드가 없을때, 부모 생성자를
            # 그대로 사용한다.!
